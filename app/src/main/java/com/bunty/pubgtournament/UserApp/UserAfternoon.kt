...
class UserAfternoon : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_user_afternoon)

        supportActionBar?.setTitle("Afternoon Match")
        getSupportActionBar()?.setDisplayHomeAsUpEnabled(true)

        progressBar = findViewById(R.id.progressBar)
        reference = FirebaseDatabase.getInstance().getReference().child("Matches").child("Afternoon")
        rvDelete = findViewById(R.id.rvAfternoon)
    ...
    private fun getData() {
        reference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                deleteAdapter = UserAdapter(deleteList, this@UserAfternoon)
            ...
            override fun onCancelled(error: DatabaseError) {
                progressBar.isVisible = false
                Toast.makeText(this@UserAfternoon, "$error", Toast.LENGTH_SHORT).show()
            }
